import { GameSlice, ValidationSlice, ValidationState } from '@/features/game/types';
...
export const validationSlice: GameSlice<ValidationSlice> = (set, get) => ({
    ...
    selectSquare: (square) => {
        ...
        const moves = chess.moves({ square, verbose: true }).map((m) => m.to as Square);
    ...
    makeMove: ({ from, to, promotion, ack = true, withSound = true, updateClock = true }) => {
